...
class _MyCarDetailsScreenState extends State<MyCarDetailsScreen> {
  ...
  late Future<CarEnquiry> _detailsFuture;

  @override
  void initState() {
    super.initState();
    // _enquiryId = (Get.arguments ?? '').toString();
    // _detailsFuture = _enquiryId.isEmpty
    //     ? Future<CarEnquiry>.error(const ApiException('Enquiry ID was missing'))
    //     : _sellCarService.fetchEnquiryDetails(_enquiryId);
  }

  Future<void> _reload() async {
    final future = _enquiryId.isEmpty
        ? Future<CarEnquiry>.error(const ApiException('Enquiry ID was missing'))
        : _sellCarService.fetchEnquiryDetails(_enquiryId);
    setState(() {
      ...
      body: FutureBuilder<CarEnquiry>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            ...
            return _DetailsErrorState(message: message, onRetry: _reload);
          }
                          ...
                          children: [
                            ...
                            _SectionCard(
                              ...
                              children: [
                                ...
                                _OverviewRow(
                                  label: 'Schedule Date',
                                  value: _formatDateTime(enquiry.scheduleDate),
                                ),
                                _OverviewRow(
                                  label: 'Schedule Time',
